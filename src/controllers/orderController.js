...
export const createOrder = async (req, res) => {
    try {
        ...
        const customerEmail = shippingInfo.email || currentUser.email || "customer@mail.com";
...
export const handlePaymentSuccess = async (req, res) => {
    ...
    try {
        const validationUrl = process.env.IS_LIVE === "true"
        ...
        console.log("🔄 Validating payment with SSLCommerz for TXN:", transactionIdString);
        ...
        if (resData.status === "VALID" ||
            ...
            console.log("✅ Payment Verified Successfully for TXN:", transactionIdString);
            res.redirect(`${process.env.FRONTEND_URL}/checkout/success?txn=${transactionIdString}`);
        }
        else {
            // গেটওয়ে থেকে কী রেসপন্স আসলো তা কনসোলে দেখা (ডিবাগিংয়ের জন্য)
            console.error("❌ SSLCommerz Validation Denied. Gateway Response:", resData);
            throw new Error(`Gateway returned status: ${resData?.status || "UNKNOWN"}`);
        }
    }
    catch (error) {
